...
public class RxConcurrentActivity extends AppCompatActivity {
    ...
    private void refresh() {
        ...
        final List<MultiHeaderEntity> list1 = new ArrayList<>();
        for (int i = 0, size = rand1 + 1; i < size; i++) {
            list1.add(new FirstItem(String.format("我是第一种类型%d--%d", i, random.nextInt(100)), timeMillis + i));
        }
        textView1.setText(String.format("类型1的数量：%d", list1.size()));

        int rand2 = random.nextInt(6);
        List<MultiHeaderEntity> list2 = new ArrayList<>();
        for (int i = 0, size = rand2 + 1; i < size; i++) {
            list2.add(new SecondItem(String.format("我是第二种类型%d--%d", i, random.nextInt(100)), timeMillis + 6 + i));
        }
        textView2.setText(String.format("类型2的数量：%d", list2.size()));

        int rand3 = random.nextInt(6);
        List<MultiHeaderEntity> list3 = new ArrayList<>();
        for (int i = 0, size = rand3 + 1; i < size; i++) {
            list3.add(new ThirdItem(String.format("我是第三种类型%d--%d", i, random.nextInt(100)), timeMillis + 12 + i));
        }
        textView3.setText(String.format("类型3的数量：%d", list3.size()));

        int rand4 = random.nextInt(6);
        List<MultiHeaderEntity> list4 = new ArrayList<>();
        for (int i = 0, size = rand4 + 1; i < size; i++) {
            list4.add(new FourthItem(String.format("我是第四种类型%d--%d", i, random.nextInt(100)), timeMillis + 18 + i));
        }
        textView4.setText(String.format("类型4的数量：%d", list4.size()));
